...
class SimpleFiLMLayer(nn.Module):
    def __init__(self, number_known_classes):
        super().__init__()
        self.conditional_vector_encoder_addition = nn.Linear(number_known_classes, 4608)
        self.conditional_vector_encoder_hadamard = nn.Linear(number_known_classes, 4608)
        #self.conv = nn.Conv2d(1024, 512, kernel_size=3, padding=1)

    def forward(self, feature_map, condition_vector):
        hadamard_tensor = (self.conditional_vector_encoder_hadamard(condition_vector)
                           .reshape((condition_vector.size(0), 512, 3, 3)))
        added_tensor = (self.conditional_vector_encoder_addition(condition_vector)
                        .reshape((condition_vector.size(0), 512, 3, 3)))
        conditioned_feature_map = torch.mul(feature_map, hadamard_tensor) + added_tensor
        #output = torch.cat([conditioned_feature_map, feature_map], dim=1)
...
class SimpleAutoencoder(nn.Module):
    def __init__(self, n_classes):
        ...
        d = 128
        self.deconv1 = nn.ConvTranspose2d(512, d * 4, 4, 1, 1)
        self.deconv1_bn = nn.BatchNorm2d(d * 4, track_running_stats=False)
        self.deconv2 = nn.ConvTranspose2d(d * 4, d * 2, 4, 2, 1)
        self.deconv2_bn = nn.BatchNorm2d(d * 2, track_running_stats=False)
        self.deconv3 = nn.ConvTranspose2d(d * 2, d//2, 4, 2, 1)
        self.deconv3_bn = nn.BatchNorm2d(d//2, track_running_stats=False)
        self.deconv4 = nn.ConvTranspose2d(d//2, 3, 4, 2, 1)

        self.film_layer = SimpleFiLMLayer(n_classes)
        self.conv1 = nn.Conv2d(3, d // 2, 4, 2, 1)
        self.conv1_bn = nn.BatchNorm2d(d // 2, track_running_stats=False)
        ...
        self.conv4 = nn.Conv2d(d * 4, 512, 4, 1, 1)
        self.conv4_bn = nn.BatchNorm2d(d * 4, track_running_stats=False)
    ...
    def forward(self, x, condition_vector):
        ...
        classification = F.leaky_relu(self.linear1(intermediate.reshape(condition_vector.size(0), 4608)), 0.2)
