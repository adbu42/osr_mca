...
show_image(test_features[0], image_dataset)
show_image(test_features[1], image_dataset)
...
# evaluation
model = C2AELightning.load_from_checkpoint(configuration['checkpoint'], n_classes=num_classes, architecture=configuration['architecture'])
...
good_prediction, good_classification = model(test_features.cuda(), conditional_vector.cuda())
bad_prediction, bad_classification = model(test_features.cuda(), non_match_conditional_vector.cuda())
show_image(good_prediction[0].cpu().detach(), image_dataset)
print(softmax(good_classification[0]))
show_image(good_prediction[1].cpu().detach(), image_dataset)
print(softmax(good_classification[1]))
show_image(bad_prediction[0].cpu().detach(), image_dataset)
show_image(bad_prediction[1].cpu().detach(), image_dataset)
